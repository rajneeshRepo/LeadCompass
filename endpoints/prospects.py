...
import os
import traceback
from schemas.prospect import ProspectsResponse, ProspectResponse
...
@router.get('/prospects/all', response_description="List of prospects", response_model=ProspectsResponse, response_model_by_alias=False, status_code=status.HTTP_200_OK)
async def get_prospects(
    ...
    try:
        ...
        prospects = list(collection_prospects.find({"module_id": ObjectId(
            module_id)}).limit(page_size).skip((page - 1) * page_size))
        total_prospects = collection_prospects.count_documents(
            {"module_id": ObjectId(module_id)})

        return ProspectsResponse(message="Prospects fetched successfully", total=total_prospects, result=prospects)
...
@router.get('/prospect/{id}', response_description="Single prospect", response_model=ProspectResponse, response_model_by_alias=False, status_code=status.HTTP_200_OK)
async def get_prospect(id: str):
    try:
        collection_prospects = get_prospects_collection()

        prospect = collection_prospects.find_one({"_id": ObjectId(id)})
        ...
        pipeline = [
            ...
            {"$group": {
                "_id": "$RcCalTransactions.DPID",
                "total_loan_count": {"$sum": 1},
                "total_loan_amount": {"$sum": "$RcCalTransactions.LoanAmount"},
                "lenders_name": {"$addToSet": "$RcCalTransactions.LenderNameBeneficiary"},
                "transactions": {"$push": "$RcCalTransactions"}
            }},
            {"$project": {
                "_id": 0,
                "DPID": "$_id",
        ...
        result = list(collection_prospects.aggregate(pipeline))

        prospect["RcCalTransactions"] = result

        return ProspectResponse(message="Prospect fetched successfully", result=prospect)
